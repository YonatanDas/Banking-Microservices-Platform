...
1. Edit the environment values file:
   ```bash
   # For dev environment
   vim helm/environments/dev-env/values.yaml
   ```
...
3. Commit and push changes:
   ```bash
   git add helm/environments/dev-env/values.yaml
...
HPA is already configured via `helm/bankingapp-common/templates/_hpa.tpl`. To adjust:
