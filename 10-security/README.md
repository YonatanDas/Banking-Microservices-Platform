...
![ESO architecture](11-docs\diagrams\ESO-diagram.png)
...
- **Implementation**: Installed via Terraform (`05-terraform/modules/eks/external_secrets.tf`), uses IRSA role (`05-terraform/modules/iam/external_secrets_role`) with least-privilege Secrets Manager read access
- **How it works**: `ClusterSecretStore` authenticates via IRSA; `ExternalSecret` resources in `06-helm/environments/*-env/templates/externalsecret.yaml` sync DB credentials to Kubernetes Secrets
...
- **Files**: `05-terraform/modules/iam/external_secrets_role/main.tf`, `06-helm/environments/*-env/templates/externalsecret.yaml`, `06-helm/environments/*-env/templates/secretstore.yaml`

---
## OIDC-based Access

- **What it protects**: Eliminates long-lived AWS credentials for CI/CD and Kubernetes service accounts
- **Implementation**: GitHub Actions uses OIDC provider (`05-terraform/modules/iam/github_oidc`) to assume `github-actions-eks-ecr-role`; Kubernetes service accounts use IRSA (EKS OIDC provider) to assume IAM roles for RDS/Secrets Manager access
...
- **Files**: `05-terraform/modules/iam/github_oidc/main.tf`, `05-terraform/modules/iam/github_oidc/github_actions_role.tf`, `.github/actions/env-setup/action.yaml`, `06-helm/environments/*-env/values.yaml` (ServiceAccount annotations)
...
- **Implementation**: NetworkPolicies generated from `06-helm/bankingapp-common/templates/_networkpolicy.tpl`; gateway allows ingress from ALB, microservices allow ingress only from gateway and explicit peer services
...
- **Files**: `06-helm/bankingapp-common/templates/_networkpolicy.tpl`, `06-helm/bankingapp-common/templates/_denyAllIngress.tpl`, `06-helm/bankingapp-common/templates/_denyAllEgress.tpl`
...
- **Implementation**: Kyverno installed via Terraform (`05-terraform/modules/eks/kyverno.tf`); ClusterPolicies in `09-kyverno/policies/cluster/` deployed via Argo CD (`07-gitops/*/applications/kyverno-policies-*.yaml`)
...
- **Files**: `09-kyverno/policies/cluster/*.yaml` (9 policies: require-cosign-signature, require-semantic-version, restrict-ecr-registry, block-privileged-containers, enforce-non-root, require-resource-limits, require-network-policy, deny-empty-env-vars, deny-hostpath-mounts)
...
- **How it works**: Scans all Terraform files in `05-terraform/` directory, checks against 1000+ policies (AWS, Kubernetes, general security), outputs SARIF format for GitHub Security tab
...
- **Files**: `.github/scripts/06-cosign-sign-verify.sh`, `09-kyverno/policies/cluster/require-cosign-signature.yaml`, `.github/workflows/Microservice-Ci.yaml`
