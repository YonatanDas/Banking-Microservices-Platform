...
Kyverno is installed via Terraform in `05-terraform/modules/eks/main.tf`:
...
- `07-gitops/dev/applications/kyverno-policies-dev.yaml`
- `07-gitops/stag/applications/kyverno-policies-stag.yaml`
- `07-gitops/prod/applications/kyverno-policies-prod.yaml`

Each Application syncs policies from `09-kyverno/policies/cluster/` to the cluster.
...
2. Validate policy syntax:
   ```bash
   for policy in 09-kyverno/policies/cluster/*.yaml; do
...
4. **Test Policy Against Resource**:
   ```bash
   kyverno test 09-kyverno/tests/ --policy 09-kyverno/policies/cluster/
   ```
...
- Update Helm values: `06-helm/environments/*/values.yaml` or `06-helm/environments/*/image-tags.yaml`
...
- If mutation fails, manually add to Helm template: `06-helm/bankingapp-common/templates/_deployment.tpl`
...
**Solution**:
- Ensure Helm values include resources: `06-helm/bankingapp-common/values.yaml`
...
1. Create policy YAML in `09-kyverno/policies/cluster/`
...
1. Edit policy YAML in `09-kyverno/policies/cluster/`
2. Validate syntax: `kyverno validate <policy-file>`
3. Test against resources: `kyverno test 09-kyverno/tests/ --policy 09-kyverno/policies/cluster/`
...
Your Helm templates in `06-helm/bankingapp-common/` already comply with most policies:
