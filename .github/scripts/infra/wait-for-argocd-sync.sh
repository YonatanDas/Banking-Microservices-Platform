...
# Waits for ArgoCD application to sync and be healthy
...
check_argocd_status() {
  ...
  if kubectl get application "${APP_NAME}" -n argocd -o json 2>/dev/null | jq -e '.status.sync.status == "Synced" and .status.health.status == "Healthy"' > /dev/null 2>&1; then
    return 0
  fi
...
while [[ ${ELAPSED} -lt ${TIMEOUT} ]]; do
  if check_argocd_status; then
    ...
    # Get detailed status
    if kubectl get application "${APP_NAME}" -n argocd -o json 2>/dev/null > /tmp/argocd-status.json; then
