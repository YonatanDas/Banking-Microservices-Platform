...
if [ -z "${SERVICE_DIR:-}" ]; then
  echo "❌ ERROR: SERVICE_DIR is not set. This should be set by initialize-service-directory.sh" >&2
  exit 1
fi
...
echo "Image built at $(date)" > build-info.txt
echo "Service: ${SERVICE}" >> build-info.txt
echo "Tag: ${GITHUB_RUN}" >> build-info.txt
echo "SHA: ${GITHUB_SHA}" >> build-info.txt
echo "✅ Build metadata saved to build-info.txt"
