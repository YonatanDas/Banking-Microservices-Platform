...
# Runs smoke tests against deployed service using health endpoints (reads from .deployment-config.yaml)
...
if [[ -f "${CONFIG_FILE}" ]]; then
  SMOKE_TESTS_ENABLED=$(yq eval ".smoke_tests.enabled // \"true\"" "${CONFIG_FILE}" 2>/dev/null || echo "true")
  
  if [[ "${SMOKE_TESTS_ENABLED}" != "true" ]]; then
    echo "⚠️  Smoke tests disabled in config, skipping"
    exit 0
  fi
  
  # Get timeout from config if not provided
  if [[ -z "${TIMEOUT}" ]]; then
    TIMEOUT=$(yq eval ".smoke_tests.timeout // 60" "${CONFIG_FILE}" 2>/dev/null || echo "60")
  fi
  
  # Get service port from config
  PORT=$(yq eval ".service_ports.${SERVICE} // \"8080\"" "${CONFIG_FILE}" 2>/dev/null || echo "8080")
  
  # Get health endpoint path from config
  HEALTH_PATH=$(yq eval ".health_endpoints.path // \"/actuator/health\"" "${CONFIG_FILE}" 2>/dev/null || echo "/actuator/health")
else
...
HEALTH_URL="http://${SERVICE_ENDPOINT}:${PORT}${HEALTH_PATH}"
...
while [[ ${RETRY_COUNT} -lt ${RETRIES} ]]; do
  if curl -sf "http://localhost:8080${HEALTH_PATH}" > /tmp/health-response.json 2>&1; then
